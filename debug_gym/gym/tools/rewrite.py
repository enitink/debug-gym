...
class RewriteTool(EnvironmentTool):
    ...
    description = (
        "Rewrite the content of the specified full file path, between lines [start, end], with the new code. Line numbers are 1-based. When start is provided and end is None, it's assumed to rewrite a single line (start). When both start and end are None, it's assumed to rewrite the whole file, this is not recommended because most of the time the expected edit is local. The new code should be valid python/c/c++ code include proper indentation (can be determined from context)."
    ...
    arguments = {
        ...
        "new_code": {
            "type": ["string"],
            "description": "The new code to be inserted. The new code should be valid python/c/c++ code include proper indentation (can be determined from context).",
        },
    }
